...
if (isset($_POST['submit'])) {
  ...
  if (count($_FILES) > 0) {
    if (is_uploaded_file($_FILES['ppicture']['tmp_name'])) {
        ...
        $query = "UPDATE usuarios SET ppicture = '$base64Image', email='$email', telefono='$telefono', dt = current_timestamp() WHERE nombre='$nombre'";
...
<script>
  function mostrarMensaje() {
    alert("Error: La imagen debe tener un tamaño máximo de 400x400 píxeles.");
  }

  function validarImagen() {
    var input = document.getElementById('ppicture');
    var img = new Image();
    img.src = window.URL.createObjectURL(input.files[0]);
    img.onload = function() {
      if (img.width > 400 || img.height > 400) {
        mostrarMensaje();
        // Restablece el valor del campo de entrada de imagen
        input.value = '';
      }
    };
  }
...
<main >
   ...
   <form action="miperfil.php" enctype="multipart/form-data" method="post" id="formulario">
    ...
    <input id="ppicture" name="ppicture" size="30" type="file" value="<?php echo $ppicture; ?>" onchange="validarImagen()" />
