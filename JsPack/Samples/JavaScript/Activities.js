...
document.addEventListener("DOMContentLoaded", function ()
{
	...
	calendar.timetableSettings.endTime = 1110;
	calendar.timetableSettings.cellTime = p.TimeSpan.fromHours(1);
	...
	nodeList = d.NodeListView.create(document.getElementById("nodeList"));
	nodeList.iconSize = new m.Drawing.Size(96, 96);
	...
	pieChart = new Controls.PieChart(pieChartEl);
	pieChart.theme.loadFrom('./Charting/Resources/DefaultExt.xml');
	...
	lineChart = new Controls.LineChart(chartEl);
	lineChart.theme.loadFrom('./Charting/Resources/DefaultExt.xml');
	...
	lineChart.plot.gridType = Charting.GridType.Horizontal;
...
function handleItemClick(sender, args)
{
	...
	var form = new CustomForm(sender, args.item);
	form.showForm();
	mk.VirtualKeyboard.find("keyboard").render();
}
...
function drawCharts()
{
	...
	if (pItems.length > 0)
	{
		var pTimes = pItems.map(function (item) { return item.endTime.valueOf() - item.startTime.valueOf() });
	...
	if (dItems.length > 0)
	{
		var dTimes = dItems.map(function (item) { return item.endTime.valueOf() - item.startTime.valueOf() });
	...
	if (rItems.length > 0)
	{
		var rTimes = rItems.map(function (item) { return item.endTime.valueOf() - item.startTime.valueOf() });
		rTotal = rTimes.reduce(function (acc, val) { return acc + val; });
		rTotal /= 3600000;
	}

	var values = new Collections.List([pTotal, dTotal, rTotal]);
	pieChart.series = new Charting.PieSeries(values, new Collections.List(["Purchases", "Deliveries", "Prints"]));
	...
	if (pItems.length > 0)
	{
		var xData = new Collections.List();
		var yData = new Collections.List();

		for (var i = 6; i < 18; i++)
		{
			var hourItems = pItems.filter(function (item) { return item.startTime.hour == i });
			xData.add(i);
			yData.add(hourItems.length);
		}

		var series1 = new Charting.Series2D(xData, yData);
	...
	if (dItems.length > 0)
	{
		...
		for (var i = 6; i < 18; i++)
		{
			var hourItems = dItems.filter(function (item) { return item.startTime.hour == i });
			xData.add(i);
			yData.add(hourItems.length);
		}

		var series2 = new Charting.Series2D(xData, yData);
	...
	if (rItems.length > 0)
	{
		...
		for (var i = 6; i < 18; i++)
		{
			var hourItems = rItems.filter(function (item) { return item.startTime.hour == i });
			xData.add(i);
			yData.add(hourItems.length);
		}

		var series3 = new Charting.Series2D(xData, yData);
