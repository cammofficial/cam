...
namespace GardenCalendar
{
	...
	function handleSelectionEnd(sender, args)
	{
		...
		var taskIndex = (document.getElementById("tasks") as HTMLInputElement).value;
	...
	{
		var cropName = (document.getElementById("crop") as HTMLInputElement).value;
		if (cropName)
		{
			resource = new p.Resource();
	...
	{
		var cropId = +(document.getElementById("crops") as HTMLInputElement).value;
	...
	function refreshCrops()
	{
		...
		var i;
		for (i = (document.getElementById("crops") as HTMLSelectElement).options.length - 1; i >= 0; i--)
		{
			(document.getElementById("crops") as HTMLSelectElement).options.item(i).remove();
		}

		for (i = 0; i < calendar.schedule.resources.count(); i++)
		{
	...
	document.getElementById("bAddCrop").addEventListener("click", addCrop);
	document.getElementById("bRemoveCrop").addEventListener("click", removeCrop);
}
