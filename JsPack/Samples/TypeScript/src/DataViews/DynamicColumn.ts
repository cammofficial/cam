...
namespace Columns {
    // states.default.forEach(state => {
    //     if (typeof state.Tag == "string") {
    //         if (!isNaN(Date.parse(state.Tag)))
    //             state.Tag = new Date(Date.parse(state.Tag))
    //     }
    // }
    // );
    ...
    grid.theme = "light";
    // set the model
    grid.model = new dv.ArrayModel(states.default.slice(0, 15), columns, "ID");
