...
namespace Navigation
{
    ...
    var item = new ui.ToolStripItem(ui.ToolStripItemType.Default);
    ...
    strip.itemClick.addEventListener(function (sender, args)
    {
        ...
        {
           selectedRow = grid.focusedCell.row;
        ...
        (<HTMLInputElement>document.getElementById("row_index")).value = selectedRow.toString();
    });
    ...
    {
        (<HTMLInputElement>document.getElementById("row_index")).value = args.row;
    });

    grid.rowDeleted.addEventListener(function (sender, args)
    {
        (<HTMLInputElement>document.getElementById("row_count")).innerHTML =
            (<HTMLInputElement>document.getElementById("row_index")).max = (sender.model.rowCount - 1).toString();
    });
    ...
    (<HTMLInputElement>document.getElementById("row_count")).innerHTML =
        (<HTMLInputElement>document.getElementById("row_index")).max = (participants.default.length - 1).toString();

    document.getElementById("row_index").addEventListener("change", function (e)
    {
        grid.bringIntoView(+(<HTMLInputElement>e.target).value).then(
            <any>grid.selectRows(+(<HTMLInputElement>e.target).value)
        );
    });

    document.getElementById("edit_mode").addEventListener("change", function (e)
    {
        grid.allowCellSelect = (<HTMLInputElement>e.target).checked;

        if ((<HTMLInputElement>e.target).checked && grid.selectedRowIndices.length)
        {
