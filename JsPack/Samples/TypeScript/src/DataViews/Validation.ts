...
namespace Validation
{
    ...
    grid.theme = "business";
    // set the model
    grid.model = new dv.ArrayModel(participants.default.slice(0,10000), columns, "index");
    ...
    grid.rowRendered.addEventListener((sender, args) =>
    {
        if (args.row < sender.model.rowCount)
            validateData(new Map((<any>Object).entries(args.rowData)), args.row);
    });
    ...
    function validateData(values, row)
    {
        ...
        if (values.has("registered"))
        {
            ...
            validateCell(row, 2, (registeredVal == undefined || registeredVal.valueOf() > new Date().valueOf()));
        }
    ...
    function validateCell(row, column, invalid)
    {
        ...
        if (cell)
        {
            ...
            else
            {
                ...
                if (cell.dataset.tooltip)
                {
                    var tooltip = <ui.Tooltip>(ui.Tooltip.find(cell.dataset.tooltip));
                    tooltip.dispose();
