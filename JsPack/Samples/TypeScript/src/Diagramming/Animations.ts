...
namespace AnimationsSample
{
    ...
    function createPulseAnimation(item)
    {
        ...
        let a = new Animation(item, options, onPulse);
        return a;
    }
    ...
    {
        let bounds = animation.item.bounds.clone();
        let to = animation.toValue;
        bounds = Drawing.DrawingUtils.inflate(bounds, to * progress, to * progress);
    ...
    function onAnimateScroll(animation, animationProgress)
    {
        ...
        let from = diagramView.scrollY;
        let to = animation.toValue;
        diagramView.scrollY = (from + (to - from) * animationProgress);
    }
