...
namespace ControlNodes
{
    ...
    const DiagramView = Diagramming.DiagramView;
    ...
    diagram.addEventListener(Diagramming.Events.nodeDomCreated, function (sender: Diagramming.Diagram, args: Diagramming.NodeEventArgs)
    {
        let node = args.node as Diagramming.ControlNode;
        let checkbox = node.getContent().querySelector("input[type='checkbox']") as HTMLInputElement;
        if (checkbox)
        {
    ...
    let nodeList = NodeListView.create(<HTMLCanvasElement>document.getElementById("nodeList"));
    nodeList.defaultNodeSize = (new Drawing.Size(20, 40));
    ...
    Controls.DomUtils.loadImageList(["../assets/icon_print.png", "../assets/icon_delivery.png", "../assets/icon_calculator.png"]).then(response =>
    {
        let node = new Diagramming.ControlNode(diagramView);
        node.setBounds(new Rect(10, 10, 100, 20));
        node.stroke = ("transparent");
        node.text = ("node");
        diagram.addItem(node);

        let node1 = new Diagramming.ControlNode(diagramView);
        node1.setBounds(new Rect(10, 50, 100, 20));
        diagram.addItem(node1);

        diagram.factory.createDiagramLink(node, node1);

        node1 = new Diagramming.ControlNode(diagramView);
        ...
        ControlNodes.arrange();
        nodeList.addNode(node1.clone(), "Print");

        let node2 = new Diagramming.ControlNode(diagramView);
        ...
        let node3 = new Diagramming.ControlNode(diagramView);
    ...
    export class ControlNodes
    {
        ...
        static placeholderClick(e, sender)
        {
            ...
            {
                Controls.DomUtils.toDataUrl(URL.createObjectURL((e.target as any).files[0]), function (base64str)
                {
                    img.src = base64str;
        ...
        static tasksMouseUp(e, sender)
        {
            let nodeList = NodeListView.find("nodeList");
            let node = nodeList.draggedNode as any;
            if (node)
            {
                let clone = node.clone();
                let bounds = sender.bounds.clone();
                bounds.x += bounds.width + 10;
                bounds.width = 20;
                bounds.height = 40;
                clone.setBounds(bounds);
                sender.parent.addItem(clone);
                let link = sender.parent.factory.createDiagramLink(sender, clone);
                link.headShape = (null);
                ControlNodes.arrange();
            }
        }

        static subordinateClick(e, sender)
        {
        ...
        static arrange()
        {
            let diagram = DiagramView.find("diagram").diagram;
            let tree = new Graphs.TreeLayout();
            tree.direction = Graphs.LayoutDirection.TopToBottom;
            tree.linkType = Graphs.TreeLayoutLinkType.Cascading;
        ...
        static toSvg()
        {
            ...
            let exporter = new Diagramming.SvgExporter();
    ...
    window["ControlNodes"] = ControlNodes;
}
